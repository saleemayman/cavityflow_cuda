...
/*			cl_int ret_val = clSetKernelArg(kernel, arg_index, sizeof(cl_int), &size);
	...
	public:
		...
		void setGridAndBlockSize(	dim3 &grid, dim3 &block, unsigned int work_dim,
									CVector<3, int> &subdomain_size, CVector<3, int> &subdomain_nums, 
		...
		)
		{
			...
			total_elements = subdomain_size.elements();

			// block = dim3(local_work_size[0], local_work_size[1], local_work_size[2]);

			// printf("cmdQ.setGridAndBlockSize() block[x, y, z]= [%i, %i, %i] \n", block.x, block.y, block.z);
			// printf("cmdQ.setGridAndBlockSize() grid[x, y, z]= [%i, %i, %i] \n", grid.x, grid.y, grid.z);

			if (work_dim == 1)
			{
				block = dim3(64, 1, 1);
				
				grids_x = total_elements/64;

				grid = dim3(grids_x + 1, 1, 1);
			}
			else if(work_dim == 2)
			{
				block = dim3(64, 64, 1);

				grids_x = sqrt(total_elements/(block.x * block.y));
				grids_y = grids_x;

				grid = dim3(grids_x + 1, grids_y + 1, 1);
			}
			else
			{
				block = dim3(64, 64, 64);

				grids_x = pow(total_elements/(block.x * block.y * block.z), 1./3.);
				grids_y = grids_x;
				grids_z = grids_x;

				grid = dim3(grids_x + 1, grids_y + 1, grids_z + 1);
			}
		...
		)
		{
		  ...
		  event = new cl_event();
			...
			// setGridAndBlockSize(grid, block, work_dim, subdomain_size, subdomain_nums, 
	 	// 						local_work_size, global_work_size);

			block = dim3(32, 8, 1);
			grid = dim3(1, 1, 1);

			// printf("cmdQ.enqueueNDRangeKernel() block[x, y, z]= [%i, %i, %i] \n", block.x, block.y, block.z);
			// printf("cmdQ.enqueueNDRangeKernel() grid[x, y, z]= [%i, %i, %i] \n", grid.x, grid.y, grid.z);
