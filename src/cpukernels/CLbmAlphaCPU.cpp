        ...
        const bool storeVelocities)
{
    for (int k = 0; k < size[2]; k++)
    {
		#pragma omp task
    	{
    		// std::cout << "thread " << omp_get_thread_num() << "/" << omp_get_num_threads() << ": I'm running iteration " << k << std::endl;

			for (int j = 0; j < size[1]; j++)
			{
				for (int i = 0; i < size[0]; i++)
				{
            		int id = (origin[2] + k) * (domainSize[0] * domainSize[1]) + (origin[1] + j) * domainSize[0] + (origin[0] + i);
